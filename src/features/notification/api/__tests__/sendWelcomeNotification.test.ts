...
beforeEach(async () => {
  ...
  const testFirestore = testEnv.authenticatedContext('alice').firestore();
  replaceDocFirestore(testFirestore as unknown as Firestore);
  replaceColFirestore(testFirestore as unknown as Firestore);
});

test('can send welcome notification to ownself', () => {
  assertSucceeds(sendWelcomeNotification('123456'));
});
